...
# Create Dataset
train_dataset = tf.data.Dataset.from_generator(input_train_gen_fn, tf.float64,
                                               output_shapes=[mnist.train.images.shape[-1]])
label_dataset = tf.data.Dataset.from_generator(input_label_gen_fn, tf.float64,
                                               output_shapes=[mnist.train.labels.shape[-1]])
dataset = tf.data.Dataset.zip((train_dataset, label_dataset)).repeat(EPOCH).batch(BATCH_SIZE)
